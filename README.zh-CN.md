...
> **当前状态：预发布技术预览版（`v0.1.0`）。** 核心本地使用流程已经可用并有自动化测试保护；GitHub Actions 现在会在推送和 PR 中验证 Node.js 18、20、22，但项目仍没有安装包、正式发布产物或完整的跨平台发布验证。
...
| 原生 `.drawio` | 只读 XML 结构预览，以及管理、链接、标签和图谱 | 仅文件名和路径 |
...
当前仓库已在 Node.js 24.15.0、npm 11.12.1 环境完成本地验证。Node.js 18+ 是预期运行范围，GitHub Actions 会自动验证 Node.js 18、20、22。
...
项目当前没有 lint 命令、安装包、桌面应用或独立服务端打包。生产前端由 Vite 构建，服务端仍通过 `tsx` 直接运行 TypeScript。
